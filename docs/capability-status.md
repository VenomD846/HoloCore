...
This page is the source of truth for what users can run now versus what feature 002 proposes.
...
| Atlas | Incremental native JSON graph, freshness, search, resolve, path, affected analysis, self-contained searchable/filterable HTML | Explicit stale/overwrite policy, edge-detail inspection, large-graph gates, broader language extraction |
...
| Client setup | Non-destructive MCP/instruction bootstrap and generated commands/skills for Codex, Claude, Gemini, Cursor, OpenCode, and generic use | Client-specific lifecycle hooks |
