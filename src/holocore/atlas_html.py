...
def _render_solar_atlas(graph: Mapping[str, Any], title: str) -> str:
    """Render the cross-World Atlas as a restrained WebGL orbital map."""
    data = _script_json(_validated_graph(graph)); safe = title.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    template = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>__TITLE__</title><style>body{{margin:0;background:#03050a;color:#d8e2f0;font:14px system-ui;overflow:hidden}}header{{height:58px;padding:0 18px;display:flex;align-items:center;gap:18px;background:#0b1020;border-bottom:1px solid #26334b}}h1{{font-size:16px;color:#9bdcff;margin:0}}#stats{{margin-left:auto;color:#8da0b8}}#stage{{height:calc(100vh - 59px);background:radial-gradient(circle,#111d37,#03050a 72%)}}canvas{{width:100%;height:100%;display:block;cursor:grab}}canvas.drag{{cursor:grabbing}}</style></head><body><header><h1>__TITLE__</h1><span id="stats"></span><span>WebGL · click a World to focus · drag to orbit · wheel to zoom</span></header><div id="stage"><canvas id="gl"></canvas></div><script id="data" type="application/json">__DATA__</script><script>
const D=JSON.parse(document.getElementById('data').textContent),c=document.getElementById('gl'),g=c.getContext('webgl');if(!g)document.getElementById('stats').textContent='WebGL unavailable';const W=(D.nodes||[]).filter(n=>n.kind==='world'),N=D.nodes||[];document.getElementById('stats').textContent=W.length+' Worlds · '+N.length+' Signals';let yaw=0,pitch=.18,scale=1,drag=null,focus=null;const vs='attribute vec3 p;attribute float s;uniform mat4 m;varying float z;void main(){gl_Position=m*vec4(p,1.);gl_PointSize=s;z=p.z;}';const fs='precision mediump float;uniform vec4 color;void main(){float d=distance(gl_PointCoord,vec2(.5));if(d>.5)discard;gl_FragColor=color;}';function sh(t,s){{const x=g.createShader(t);g.shaderSource(x,s);g.compileShader(x);return x}}const pr=g.createProgram();g.attachShader(pr,sh(g.VERTEX_SHADER,vs));g.attachShader(pr,sh(g.FRAGMENT_SHADER,fs));g.linkProgram(pr);const P=g.getAttribLocation(pr,'p'),S=g.getAttribLocation(pr,'s'),M=g.getUniformLocation(pr,'m'),C=g.getUniformLocation(pr,'color');function mat(){{const a=Math.cos(yaw),b=Math.sin(yaw),cp=Math.cos(pitch),sp=Math.sin(pitch);return new Float32Array([a,sp*b,-cp*b,0,0,cp,sp,0,b,-sp*a,cp*a,0,0,0,-.002*scale,1])}}function draw(t){{c.width=c.clientWidth*devicePixelRatio;c.height=c.clientHeight*devicePixelRatio;g.viewport(0,0,c.width,c.height);g.clearColor(.01,.02,.04,1);g.clear(g.COLOR_BUFFER_BIT);g.useProgram(pr);g.uniformMatrix4fv(M,false,mat());const pts=[];W.forEach((w,i)=>{{const a=i/W.length*6.283+t*.00008,r=focus&&focus.id===w.id?.0:0.65,x=Math.cos(a)*r,y=Math.sin(a)*r*.62,z=Math.sin(a*.7)*.18;pts.push(x,y,z,18,w.id===focus?.id?1:0.55);for(let j=0;j<Math.min(90,N.filter(n=>n.world===w.world).length);j++){{const q=j/90*6.283+t*.00035,rr=focus&&focus.id===w.id?.34:.13;pts.push(x+Math.cos(q)*rr,y+Math.sin(q)*rr*.62,z+Math.sin(q*1.7)*.08,focus&&focus.id===w.id?4:2,1)}}}});const b=g.createBuffer();g.bindBuffer(g.ARRAY_BUFFER,b);g.bufferData(g.ARRAY_BUFFER,new Float32Array(pts),g.DYNAMIC_DRAW);g.enableVertexAttribArray(P);g.vertexAttribPointer(P,3,g.FLOAT,false,20,0);g.enableVertexAttribArray(S);g.vertexAttribPointer(S,1,g.FLOAT,false,20,12);g.uniform4f(C,.35,.78,1,1);g.drawArrays(g.POINTS,0,pts.length/5);requestAnimationFrame(draw)}}function hit(){{focus=focus?null:W[0];}}c.onclick=hit;c.onwheel=e=>{{scale=Math.max(.45,Math.min(2.4,scale*(e.deltaY<0?1.1:.9)))}};c.onpointerdown=e=>{{drag={{x:e.clientX,y:e.clientY}};c.classList.add('drag')}};c.onpointermove=e=>{{if(drag){{yaw+=(e.clientX-drag.x)*.006;pitch+=(e.clientY-drag.y)*.004;drag={{x:e.clientX,y:e.clientY}}}}}};c.onpointerup=()=>{{drag=null;c.classList.remove('drag')}};requestAnimationFrame(draw);
</script></body></html>'''
    return template.replace("__TITLE__", safe).replace("__DATA__", data).replace("{{", "{").replace("}}", "}")
