...
def _render_solar_atlas(graph: Mapping[str, Any], title: str) -> str:
    """Render the cross-World Atlas as a self-contained 3D WebGL solar system.

    Worlds are the large bodies, their Signals orbit their parent World, and
    structural links are rendered as faint constellation lines.  The viewer
    deliberately keeps the graph bounded and deterministic so it remains useful
    for large Obsidian-derived graphs without requiring a remote runtime.
    """
    data = _script_json(_validated_graph(graph))
    safe = title.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    template = r'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="holocore-atlas-viewer" content="4"><title>__TITLE__</title><style>
:root{color-scheme:dark;font:14px "IBM Plex Sans","Segoe UI",system-ui,sans-serif}*{box-sizing:border-box}body{margin:0;background:#02050b;color:#dbeafe;overflow:hidden}header{height:64px;padding:10px 18px;display:flex;align-items:center;gap:16px;background:#0b1222e8;border-bottom:1px solid #233451;box-shadow:0 8px 28px #0008;position:relative;z-index:2}h1{font-size:16px;color:#8ed8ff;margin:0;letter-spacing:.02em}#stats{margin-left:auto;color:#9aacca}#hint{color:#7285a1;font-size:12px}#stage{position:relative;height:calc(100vh - 64px);background:radial-gradient(ellipse at 50% 48%,#132343 0,#081226 37%,#02050b 80%)}canvas{width:100%;height:100%;display:block;cursor:grab}canvas.drag{cursor:grabbing}#worlds{position:absolute;left:16px;top:16px;display:flex;flex-direction:column;gap:6px;max-height:calc(100% - 32px);overflow:auto;width:230px;padding:10px;border:1px solid #273c5d;border-radius:10px;background:#081226bb;backdrop-filter:blur(8px)}#worlds button{border:1px solid #2b4266;border-radius:7px;background:#0d1a30;color:#cfe7ff;text-align:left;padding:8px 10px;cursor:pointer}#worlds button:hover,#worlds button.active{border-color:#49c8ff;background:#123252}#worlds small{display:block;color:#7e97b8;margin-top:2px}.badge{font:11px "JetBrains Mono",monospace;color:#8eb7d8;margin-bottom:3px}</style></head><body><header><h1>__TITLE__</h1><span id="stats"></span><span id="hint">click a World to zoom · drag to orbit · wheel to zoom</span></header><div id="stage"><nav id="worlds" aria-label="World switcher"></nav><canvas id="gl" aria-label="3D HoloCore solar system"></canvas></div><script id="data" type="application/json">__DATA__</script><script>
(()=>{'use strict';const D=JSON.parse(document.getElementById('data').textContent),W=(D.nodes||[]).filter(n=>String(n.kind||'').toLowerCase()==='world'),N=D.nodes||[],L=D.links||D.edges||[],canvas=document.getElementById('gl'),stats=document.getElementById('stats'),list=document.getElementById('worlds');stats.textContent=`${W.length} Worlds · ${N.length} Signals · ${L.length} links`;
const gl=canvas.getContext('webgl2',{antialias:true,alpha:false})||canvas.getContext('webgl',{antialias:true,alpha:false});if(!gl){stats.textContent+=' · WebGL unavailable';return}gl.enable(gl.DEPTH_TEST);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);const vs=`attribute vec3 a;attribute float s;attribute vec3 col;uniform mat4 u;varying vec3 v;void main(){gl_Position=u*vec4(a,1.0);gl_PointSize=s;v=col;}`,fs=`precision mediump float;varying vec3 v;void main(){float d=distance(gl_PointCoord,vec2(.5));if(d>.5)discard;float glow=1.0-smoothstep(.12,.5,d);gl_FragColor=vec4(v,glow);}`;function compile(type,src){const s=gl.createShader(type);gl.shaderSource(s,src);gl.compileShader(s);if(!gl.getShaderParameter(s,gl.COMPILE_STATUS))throw Error(gl.getShaderInfoLog(s));return s}const program=gl.createProgram();gl.attachShader(program,compile(gl.VERTEX_SHADER,vs));gl.attachShader(program,compile(gl.FRAGMENT_SHADER,fs));gl.linkProgram(program);if(!gl.getProgramParameter(program,gl.LINK_STATUS))throw Error(gl.getProgramInfoLog(program));const A=gl.getAttribLocation(program,'a'),S=gl.getAttribLocation(program,'s'),COL=gl.getAttribLocation(program,'col'),U=gl.getUniformLocation(program,'u');
const hash=s=>{let h=2166136261;for(const c of String(s)){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}return h>>>0},byWorld=new Map(W.map(w=>[String(w.world||w.id),w])),signals=new Map(W.map(w=>[String(w.world||w.id),N.filter(n=>String(n.world||'')===String(w.world||w.id)&&n.id!==w.id)])),centers=new Map(),selected=null;W.forEach((w,i)=>{const id=String(w.world||w.id),a=i/W.length*Math.PI*2,r=1.35+Math.sqrt(i+1)*.22;centers.set(id,{x:Math.cos(a)*r,y:(hash(id)%100)/500-.1,z:Math.sin(a)*r})});
W.forEach(w=>{const id=String(w.world||w.id),b=document.createElement('button');b.dataset.world=id;b.innerHTML=`<span>${String(w.label||id)}</span><small>${(signals.get(id)||[]).length} Signals</small>`;b.onclick=()=>select(id,true);list.appendChild(b)});
let yaw=.15,pitch=.28,distance=4.8,targetDistance=distance,drag=null,focus=null,down=null,raf=0;const perspective=(fovy,aspect,near,far)=>{const f=1/Math.tan(fovy/2),nf=1/(near-far);return new Float32Array([f/aspect,0,0,0,0,f,0,0,0,0,(far+near)*nf,-1,0,0,2*far*near*nf,0])};const mul=(a,b)=>{const o=new Float32Array(16);for(let c=0;c<4;c++)for(let r=0;r<4;r++)o[c*4+r]=a[r]*b[c*4]+a[4+r]*b[c*4+1]+a[8+r]*b[c*4+2]+a[12+r]*b[c*4+3];return o};const view=()=>{const cp=Math.cos(pitch),sp=Math.sin(pitch),cy=Math.cos(yaw),sy=Math.sin(yaw),x=distance*cp*sy,y=distance*sp,z=distance*cp*cy;return new Float32Array([cy,sp*sy,-cp*sy,0,0,cp,sp,0,sy,-sp*cy,cp*cy,0,-x,-y,-z,1])};function select(id,animate){selected=id;focus=centers.get(id)||null;targetDistance=animate?2.5:distance;list.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.world===id));if(id){const w=byWorld.get(id);stats.textContent=`${w.label||id} · ${(signals.get(id)||[]).length} Signals · click another World to switch`}}
function project(p,m){const x=m[0]*p.x+m[4]*p.y+m[8]*p.z+m[12],y=m[1]*p.x+m[5]*p.y+m[9]*p.z+m[13],z=m[3]*p.x+m[7]*p.y+m[11]*p.z+m[15];return{x:(x/z*.5+.5)*canvas.clientWidth,y:(-y/z*.5+.5)*canvas.clientHeight,z}}function frame(time){raf=requestAnimationFrame(frame);const dpr=Math.min(2,devicePixelRatio||1),w=canvas.clientWidth*dpr,h=canvas.clientHeight*dpr;if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h)}distance+=(targetDistance-distance)*.08;gl.clearColor(.005,.009,.02,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);const m=mul(perspective(Math.PI/3,w/h,.05,40),view()),pts=[],stride=7;W.forEach((world,i)=>{const id=String(world.world||world.id),c=centers.get(id),active=id===selected;const count=Math.min(180,(signals.get(id)||[]).length),wa=time*.00006*(i%2?-1:1);pts.push(c.x,c.y,c.z,active?31:24,.28,.75,1);for(let j=0;j<count;j++){const q=j/count*Math.PI*2+wa,rr=.22+.015*(j%7),z=c.z+Math.sin(q*3+hash(id)%8)*.08;pts.push(c.x+Math.cos(q)*rr,c.y+Math.sin(q)*rr*.64,z,active?6:3.2,.24+((j*13)%50)/200,.58+((j*7)%40)/200,1)}});const b=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,b);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(pts),gl.DYNAMIC_DRAW);gl.useProgram(program);gl.uniformMatrix4fv(U,false,m);gl.enableVertexAttribArray(A);gl.vertexAttribPointer(A,3,gl.FLOAT,false,stride*4,0);gl.enableVertexAttribArray(S);gl.vertexAttribPointer(S,1,gl.FLOAT,false,stride*4,12);gl.enableVertexAttribArray(COL);gl.vertexAttribPointer(COL,3,gl.FLOAT,false,stride*4,16);gl.drawArrays(gl.POINTS,0,pts.length/stride);gl.deleteBuffer(b)}
function hit(e){const rect=canvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top,m=mul(perspective(Math.PI/3,canvas.clientWidth/canvas.clientHeight,.05,40),view());let best=null,dist2=22*22;W.forEach(w=>{const p=project(centers.get(String(w.world||w.id)),m),dx=p.x-x,dy=p.y-y;if(dx*dx+dy*dy<dist2){best=String(w.world||w.id);dist2=dx*dx+dy*dy}});if(best)select(best,true);else if(selected)select(null,false)}canvas.addEventListener('click',e=>{if(!down||Math.hypot(e.clientX-down.x,e.clientY-down.y)<5)hit(e)});canvas.addEventListener('pointerdown',e=>{down={x:e.clientX,y:e.clientY};drag={x:e.clientX,y:e.clientY};canvas.classList.add('drag');canvas.setPointerCapture(e.pointerId)});canvas.addEventListener('pointermove',e=>{if(!drag)return;yaw+=(e.clientX-drag.x)*.006;pitch=Math.max(-1.2,Math.min(1.2,pitch+(e.clientY-drag.y)*.004));drag={x:e.clientX,y:e.clientY}});canvas.addEventListener('pointerup',()=>{drag=null;canvas.classList.remove('drag')});canvas.addEventListener('wheel',e=>{e.preventDefault();targetDistance=Math.max(1.6,Math.min(10,targetDistance*(e.deltaY<0?.88:1.14)))},{passive:false});canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();cancelAnimationFrame(raf);stats.textContent+=' · context lost; release to restore'});canvas.addEventListener('webglcontextrestored',()=>{location.reload()});frame(0);
})();</script></body></html>'''
    return template.replace("__TITLE__", safe).replace("__DATA__", data)
