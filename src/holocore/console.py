...
def build_console_payload(root: Path) -> dict:
    ...
    if atlas["exists"]:
        try:
            graph = json.loads(Path(paths["atlas_json"]).read_text(encoding="utf-8"))
...
<script>const S=JSON.parse(document.getElementById('state').textContent);const tabs=['Overview','Chat Deck','Animus','Archive','Atlas','Locations','AI Commands','Maintenance'];let current='Overview';
